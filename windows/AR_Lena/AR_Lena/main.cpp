...
//{
    int camnum = 0;//配布0,リアル2,仮想6
    if(__argc > 1)
        camnum = std::stoi(__argv[1]);
    //if (argc > 1)
    ...
    while (cap.read(image)) {
        ...
        /*bool data = detector.detect(input_image, points);
         ...
         //cout << points << endl;
            ...
            if (data == true) {
                ...
                for (size_t i = 0; i < points.size(); ++i) {
                 cv::line(output_image, points[i], points[(i + 1) % points.size()], cv::Scalar(255, 0, 0), 4);
                }
         }
         */
            ...
            if (data.length() > 0) {
                ...
                // 検出結果の矩形描画
                for (size_t i = 0; i < points.size(); ++i) {
                    cv::line(output_image, points[i], points[(i + 1) % points.size()], cv::Scalar(0, 255, 0), 4);
                }
            }
            else {
